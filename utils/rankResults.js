 ...
 */
function LCSubStr( X,  Y , m = X.length , n = Y.length) {
...
const rankResults = (results, term) => {
    const withRank = results.map(result => {
      const rank = LCSubStr(term.toLowerCase(), result.name.toLowerCase());
      return { ...result, rank };
    });
